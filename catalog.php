        ...
        <!--FILTERS-->
                <?php
                    /*Panavigation code*/
                        ...
                        $count = mysqli_query ($connection, "SELECT COUNT(*) FROM products");
                    ...
                    $product_num = mysqli_query ($connection, "SELECT * FROM products ORDER BY $sorting");
                    $row = mysqli_num_rows($product_num);

                    $product = mysqli_query ($connection, "SELECT * FROM products ORDER BY $sorting $query_start_num");
                ?>
                ...
                <!--sorting-->
                        ...
                        <ul>
                            ...
                            <li id="select-sort"><?php echo $sort_name;?>
                                ...
                                <ul id="sorting-list">
                                    <li><a href=catalog.php?sort=price-asc#bc>cheap to expensive</a></li>
                                    <li><a href=catalog.php?sort=price-desc#bc>expensive to cheap</a></li>
                                    <li><a href=catalog.php?sort=popular#bc>popular</a></li>
                                    <li><a href=catalog.php?sort=news#bc>news</a></li>
                                    <li><a href=catalog.php?sort=brand#bc>a to z</a></li>
                                </ul>
                            </li>
                        </ul>     
                    </div>
            ...
            <!--PAGINATION-->     
                    <div class="pagination section section-color pagenav">
                            <!--previous-->
                                        <?php if ($page != 1) { $pstr_prev = '<li class="page-item"><a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page - 1).'#bc" aria-label="Previous"><span aria-hidden="true">&laquo;</span><span class="sr-only">Previous</span></a></li>' ;} ?>
                            <!--next-->
                                        <?php if ($page != $total) { $pstr_next = '<li class="page-item">
                                            <a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page + 1).'#bc" aria-label="Next"><span aria-hidden="true">&raquo;</span><span class="sr-only">Next</span></a></li>' ;} ?>
                            <!--pages-->
                                        <?php 
                                        
                                        if ($page - 3 > 0) $page3left = '<li class="page-item"><a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page - 3).'#bc">'.($page - 3).'</a></li>' ; 
                                        if ($page - 2 > 0) $page2left = '<li class="page-item"><a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page - 2).'#bc">'.($page - 2).'</a></li>' ;
                                        if ($page - 1 > 0) $page1left = '<li class="page-item"><a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page - 1).'#bc">'.($page - 1).'</a></li>' ; 
                                        
                                        
                                        if ($page + 3 <= $total) $page3right = '<li class="page-item"><a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page + 3).'#bc">'.($page + 3).'</a></li>' ; 
                                        if ($page + 2 <= $total) $page2right = '<li class="page-item"><a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page + 2).'#bc">'.($page + 2).'</a></li>' ; 
                                        if ($page + 1 <= $total) $page1right = '<li class="page-item"><a class="page-link" href="catalog.php?sort='.$sort.'&page='.($page + 1).'#bc">'.($page + 1).'</a></li>' ; 

                                        if ($page + 3 > $total) {
                                            $strtotal = '<li><p class="nav-point">...</p></li><li><a href="catalog.php?sort='.$sort.'&page='.$total.'">'.$total.'</a></li>';
                                        } else {
                                            $strtotal = "";
                                        }
                                        if ($total > 1) {
                                            echo '<nav aria-label="Page navigation example"><ul class="pagination">'.$pstr_prev.$page3left.$page2left.$page1left.'<li class="page-item"><a class="page-link active-page" href="catalog.php?sort='.$sort.'&page='.$page.'#bc">'.$page.'</a></li>'.$page1right.$page2right.$page3right.$pstr_next.'</ul></nav>';  }
                                        ?>
